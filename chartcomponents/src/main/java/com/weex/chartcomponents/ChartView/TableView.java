...
public class TableView extends LinearLayout {
    ...
    protected TableView(Context context) {
        super(context);
        initView(context);
...
//        table = (TableFixHeaders)findViewById(R.id.Table);
    ...
    public void make_matrixTableAdapter(Data_collection datas){
...
//        }
        getShowSS(datas);
        matrixTableAdapter = new MatrixTableAdapter<String>(context, show_ss);
        table.setAdapter(matrixTableAdapter);
    }

    public void resetData(Data_collection datas){
//        show_ss = new String[datas.getXString().length+1][datas.getDataset().size()+1];
//
//        show_ss[0][0]="时间";
//        for(int i=0;i<datas.getXString().length;i++){
